...
def askChatGPT(texts):
    ...
    os.chdir('.')
    vecList = pickle.load(open('ja_en_chatgpt_vectorList.pl','rb'))
    start=len(vecList)
    tap=len(texts)//70
    end=len(texts)
    while start<end:
        ...
        pickle.dump(vecList,open('ja_en_chatgpt_vectorList.pl', 'wb'))
        print(start, start1)
        start=start1
        time.sleep(20)
...
def test():
    ...
    embedding_list=pickle.load(open('fr_en_chatgpt_vectorList.pl','rb'))
    print(len(embedding_list),len(embedding_list[0]))
    input_embeddings =torch.tensor(embedding_list) #F.normalize(torch.tensor(embedding_list), p=2, dim=1)
    embedding = input_embeddings
    embedding[torch.isnan(embedding)] = 0
    get_hits(test_ill, F.normalize(embedding, p=2,dim=1), True, [1,50])
...
def get_name(name):
    ...
    return sub_string
id2comment=load_id2object(['DBP15k/ja_en/comment_1','DBP15k/ja_en/comment_2'])
id2name=load_id2object(['DBP15k/ja_en/ent_ids_1','DBP15k/ja_en/ent_ids_2'],"entity")
texts=[]
for i in range(len(id2name)):
    comment=get_name(id2name[i])+': '+id2comment[i] if i in id2comment else get_name(id2name[i])
    texts.append(comment)
pickle.dump([], open('ja_en_chatgpt_vectorList.pl', 'wb'))
askChatGPT(texts)
