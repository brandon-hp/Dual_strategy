...
import torch
import copy
from Test import get_hits,get_hits_b
sys.path.append('..')
...
class RelationDataset(object):
    ...
    def __next__(self):
        if self.iter_count < self.batch_num:
            ...
            pn1 = self.all_neighbor.get(pe1)
            pn2 = self.all_neighbor.get(pe2)
            nn1 = self.all_neighbor.get(ne1)
            nn2 = self.all_neighbor.get(ne2)
